...
public class FMODEditorExtension : MonoBehaviour
{
	...
	static bool CreateSystem()
	{
    	...
    	if (!ERRCHECK(FMOD.Studio.Factory.System_Create(out sFMODSystem)))
		{
			return false;
		}

		// Note plugins wont be used when auditioning inside the Unity editor
		ERRCHECK(sFMODSystem.init(256, FMOD.Studio.INITFLAGS.ALLOW_MISSING_PLUGINS, FMOD.INITFLAGS.NORMAL, System.IntPtr.Zero));
...
#if false
		int prevCount = 0;
		while (prevCount < loadedBanks.Count)
		{
			int count = 0;
			foreach(var bank in loadedBanks)
			{
				FMOD.Studio.LOADING_STATE state;
				if (ERRCHECK(bank.getLoadingState(out state)))
				{
					FMOD.GUID id;
					bank.getID(out id);
					FMOD.Studio.UnityUtil.LogError("An error occured while loading bank with ID: "
						+ new System.Guid((int)id.Data1, (short)id.Data2, (short)id.Data3, id.Data4).ToString("B"));
					return false;
				}
			...
			if (count != prevCount)
			{
				float progress = count / (float)loadedBanks.Count;
				
				EditorUtility.DisplayProgressBar("FMOD Studio Bank Importer", "Loading banks", progress);
				prevCount = count;
			}
		}
